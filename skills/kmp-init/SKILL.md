...
Creates a **runnable, self-contained** Kotlin Multiplatform application from the kit's
...
`kmp-ai-kit` — the kit's CLI. One subcommand:

```bash
kmp-ai-kit new GithubLeaderboard com.example.demo
kmp-ai-kit new MyApp com.acme.myapp ~/projects/MyApp
kmp-ai-kit help
```

Positionals: `<Name> <pkg> [dest]`. Without a `dest` the app is created next to the kit
(`<kit>/../<Name>`). Flags `--name/--pkg/--dest` still work and win when given. The
`kmp-init` bin and `npm run init` are aliases. The template is `templates/kmp-project/`;
the verifier is `scripts/dev/e2e-check.mjs`. No MCP.

**Self-contained by default.** The scaffold **copies** the kit runtime into the project —
`skills/` (minus `kmp-init`), `shared/`, `policies/`, `prompts/`, `workflows/`, `docs/`,
`AGENTS.md`, and `.opencode/plugins/protect-feature.ts` — and `opencode.json` references
only project-local paths. The result depends on **no path outside itself**: it survives a
move, a clone, or being handed to someone else, and `kmp-ai-kit` need not be installed.

`--linked` opts into the opposite trade-off: thin **relative** links to the kit instead of
copies, so there is one source of truth (a kit update reaches the project) but the project
and the kit must move together. Never hand-write an absolute path into `opencode.json` —
that pins the project to one machine.
...
| Package | positional 2 (or `--pkg`) | lowercase dotted, at least two segments, e.g. `com.example.demo` |
| Linkage | `--linked` | self-contained copies by default; `--linked` for relative refs to the kit |

Objective: one runnable, self-contained app. Acceptance criteria: the script reports
success; the checker passes; no template identifier (`dev.kmpapp`, `KmpApp`) remains; the
project depends on no path outside itself; the repo is git-initialized.
...
- **Phase 1 — dry run.** `kmp-ai-kit new <Name> <pkg> --dry-run` and report the file count
  and destination it would write. **Exit:** the user confirms.
- **Phase 2 — scaffold.** Run `kmp-ai-kit new <Name> <pkg> [dest]`. Report the file count
  and rewrite count it prints. OpenSpec is initialized by default; `--no-openspec` skips it.
- **Phase 3 — verify.** From the new project:
  ...
  tree for `dev.kmpapp` / `KmpApp` — expect zero hits — and for the kit's absolute path,
  which must not appear anywhere. Confirm `opencode debug skill` in the project reports
  the `kmp-*` skills, and that `git log` shows the initial commit. **✋ Checkpoint:** show
...
8. **No path outside the project.** Self-contained mode copies the runtime in; `--linked`
   uses kit-relative paths. An absolute path in `opencode.json` is a bug, not a
   convenience. `kmp-init` itself is excluded from the copied skills, and its routes are
   removed from the copied router and routing table.
...
| "An absolute path in `opencode.json` is simpler." | It pins the project to one machine and breaks on any move, clone, or share. | Keep self-contained mode (project-local paths) or use `--linked` (kit-relative). Never hand-write one. |
...
```bash
# one-time, so `kmp-ai-kit` is on PATH (from the kit root)
npm link

# the canonical invocation
kmp-ai-kit new GithubLeaderboard com.example.demo

# dry-run first, or scaffold somewhere specific
kmp-ai-kit new MyApp com.acme.myapp --dry-run
kmp-ai-kit new MyApp com.acme.myapp ~/projects/MyApp

# verify (from the new project)
python3 shared/scripts/kmp_check.py --all
node <kit>/scripts/dev/e2e-check.mjs --root .
opencode debug skill | grep -E 'kmp-|penpot-'   # the skills resolve in the project
git log --oneline
```
